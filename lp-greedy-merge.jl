...
function load_potential_facilities(country)
    ...
    potential_locations = Dict{Int, Vector{Tuple{Int, Float64}}}()
    for k in 1:length(clients_col)
        i = clients_col[k]
        j = facilities_col[k]
        t = t_ij_col[k]
        if !haskey(potential_locations, i)
            potential_locations[i] = Tuple{Int, Float64}[]
        end
        push!(potential_locations[i], (j, t))
    end

    return (; potential_facs, potential_locations)
end
...
function drop_heuristic(open_set, min_students, w, facility_cost, data, use_power_law,
                        init_assigned=nothing, init_cur_cost=nothing, init_cur_time=nothing)
    (; facilities, facilities_col, t_ij_col, locations, client_pop) = data
    open_set          = copy(open_set)
    existing_set      = Set(facilities)
    potential_in_open = Set(j for j in open_set if !(j in existing_set))
    ...
    # Carry forward assignments to potential facilities from the previous merge step
    if init_assigned !== nothing
        for (i, j) in init_assigned
            if j in potential_in_open
                assigned[i] = j
                cur_cost[i] = init_cur_cost[i]
                cur_time[i] = init_cur_time[i]
                fload[j]   += client_pop[i]
            end
        end
    end

    # Assign remaining clients via existing OD
    for (i, rows) in locations
        haskey(assigned, i) && continue
        best_k    = nothing
        best_cost = Inf
        for k in rows
            j = facilities_col[k]
    ...
    facility_cost_cache = Dict(j => facility_penalty(fload[j], min_students, w, facility_cost, use_power_law) for j in open_set)
    ...
    while improved
        ...
        for j in open_set
            j in potential_in_open && continue  # only existing facilities are candidates to drop
...
function merge_heuristic(open_set, assigned, fload, cur_cost, cur_time,
                         min_students, w, facility_cost, data, pot_data, use_power_law)

    (; facilities, facilities_col, t_ij_col, locations, client_pop) = data
    (; potential_facs, potential_locations) = pot_data

    open_set = copy(open_set)
    assigned = copy(assigned)
    fload    = copy(fload)
    cur_cost = copy(cur_cost)
    cur_time = copy(cur_time)

    facility_clients = Dict(j => Int[] for j in union(Set(facilities), Set(potential_facs)))
    for (i, j) in assigned
        push!(facility_clients[j], i)
    end
    ...
    # Precompute for each potential facility p: which clients have an OD entry to it
    potential_client_set = Dict(p => Set{Int}() for p in potential_facs)
    for (i, entries) in potential_locations
        for (j, _) in entries
            if haskey(potential_client_set, j)
                push!(potential_client_set[j], i)
            end
        end
    end

    # Precompute (client, potential facility) → min travel time for O(1) lookup in inner loop
    ...
    # Step 1: candidate pairs via shared second-best
        pair_overlap = Dict{Tuple{Int,Int}, Int}()
        for j_a in open_set
            for i in facility_clients[j_a]
                best_alt_j = -1
                best_alt_c = Inf
                for k in locations[i]
                    j_cand = facilities_col[k]
                    if j_cand != j_a && j_cand in open_set
                        ck = c(t_ij_col[k])
                        if ck < best_alt_c
                            best_alt_c = ck
                            best_alt_j = j_cand
                ...
                if best_alt_j != -1
                    key = minmax(j_a, best_alt_j)
                    pair_overlap[key] = get(pair_overlap, key, 0) + 1
                end
            end
        end

        candidate_pairs = collect(keys(pair_overlap))

        # Precompute for each open facility: which potentials have OD to any of its clients
        ...
        # Step 2: for each candidate pair, evaluate only potentials reachable from both sides
        best_saving = 0.0
        best_move   = nothing  # (j_a, j_b, p, new_assignments)

        for (j_a, j_b) in candidate_pairs
            candidate_potentials = intersect(fac_to_potentials[j_a], fac_to_potentials[j_b])
        ...
        if best_move !== nothing
            ...
            for i in combined_clients
                ...
                # clients with no OD entry to p keep their current assignment
                # (cur_cost/cur_time unchanged, they will be re-optimised by drop_heuristic)
            end
    ...
    return open_set, assigned, fload, cur_cost, cur_time, travel_cost, penalty, any_merged
end
...
function run_scenario(data, pot_data, min_students, w, use_power_law)
    ...
    println("    initial drop → $(length(open_set)) existing open")

    merge_iter = 0
    any_merged = true
    while any_merged
        open_set, assigned, fload, cur_cost, cur_time, travel_cost, penalty, any_merged =
            merge_heuristic(open_set, assigned, fload, cur_cost, cur_time,
                            min_students, w, facility_cost, data, pot_data, use_power_law)
        if any_merged
            merge_iter += 1
            open_set, assigned, fload, cur_cost, cur_time, travel_cost, penalty =
                drop_heuristic(open_set, min_students, w, facility_cost, data, use_power_law,
                               assigned, cur_cost, cur_time)
            n_existing  = sum(1 for j in open_set if j in Set(facilities); init=0)
            n_potential = length(open_set) - n_existing
            # println("    merge $merge_iter → $n_existing existing + $n_potential potential open")
        end
    end 
        ...
        for use_power_law in use_power_laws
            ...
            for min_students in min_students_values
                for w in ws
                    ...
                    local open_set, assigned, fload, cur_cost, cur_time, travel_cost, penalty, raw_penalty, n_open_full, n_open_small, mean_travel_min =
...
function single_run(country, min_students, w, use_power_law)
    ...
    open_set, assigned, fload, cur_cost, cur_time, travel_cost, penalty, raw_penalty, n_open_full, n_open_small, mean_travel_min =
        run_scenario(data, pot_data, min_students, w, use_power_law)
    ...
    # Potential facilities — only those opened by merge
    pot_open_ids  = Int[]
    pot_open_flag = Int[]
    for j in potential_facs
        if j in open_set
            push!(pot_open_ids,  j)
            push!(pot_open_flag, isinf(min_students) || fload[j] >= min_students ? 1 : 2)
        end
    end
