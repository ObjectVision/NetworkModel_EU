...
set_optimizer_attribute(model, "presolve", "on") 
 ...
 # variables 
 @variable(model, 0 <= y[1:N] <= 1) # per od pair 
 @variable(model, 0 <= x[j in facilities] <= 1) # per facility 
 @variable(model, deficit[j in facilities] >= 0) 
 
 @expression(model, load[j in facilities], sum(y[k] * population[clients_col[k]+1] * 0.1 for k in facility_rows[j]) ) 
...
for j in facilities 
    @constraint(model, deficit[j] >= min_students*x[j] - load[j]) # deficit[j] is 0 if load ≥ min_students, min_students-load if load < min_students 
end 
