...
$charts = @(
  ...
  @{f='doc\charts\cross_lambda_map_LINEAR_slide.png';   t='Cross-lambda per study area on the map  -  LINEAR';   r=$XMAP; n='Green = low cross-lambda (a location buys little travel), red = high; EUR per location via the placeholder EUR 100,000 (= w x 100,000), so read the ranking, not the amounts. FR/IT/SE/PL are drawn as their NUTS-1 regions, Portugal as its mainland; grey = not in the study. Rendered by GeoDMS 20.19 from cfg\cross_lambda_map.dms on the NUTS 2021 boundaries (EPSG:3035); the same classes as the ranked list two slides back.'},
  @{f='doc\charts\cross_lambda_map_LOGISTIC_slide.png'; t='Cross-lambda per study area on the map  -  LOGISTIC'; r=$XMAP; n='Same map for the logistic cost: its lambda is on its own scale (the cost c(t) is dimensionless, so lambda per location is about 1/100 of the linear one), hence classes at one hundredth of the linear breaks; compare areas, not functions. The picture is the one of the linear cost with moves at the class edges: Belgium and the north of Italy stay lowest, the Nordic areas highest, and Austria and Finland join the top class.'},
  @{f='doc\charts\xlambda_scatter_LINEAR.png'; t='Cross-lambda against residents per location, by OECD archetype  -  LINEAR'; r=$CLOUD; n='x = residents per 1 km2 pharmacy location today (the count the frontier uses), y = cross-lambda, both on log scales. The dotted line is the log-log least-squares fit over all areas - the part of cross-lambda that today''s density predicts; the distance to it is what is left for geography and policy. Archetype colours as on the map of Lewis''s benchmarking deck (p22-23). Residuals per area: doc\cross_lambda_residuals_<FN>.csv (doc\cross_lambda_scatter.py).'},
  @{f='doc\charts\xlambda_scatter_LOGISTIC.png'; t='Cross-lambda against residents per location, by OECD archetype  -  LOGISTIC'; r=$CLOUD; n='x = residents per 1 km2 pharmacy location today (the count the frontier uses), y = cross-lambda, both on log scales. The dotted line is the log-log least-squares fit over all areas - the part of cross-lambda that today''s density predicts; the distance to it is what is left for geography and policy. Archetype colours as on the map of Lewis''s benchmarking deck (p22-23). Residuals per area: doc\cross_lambda_residuals_<FN>.csv (doc\cross_lambda_scatter.py).'},
