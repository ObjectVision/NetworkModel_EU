...
| **Max cap** | Max catchment-population per facility, from the observed distribution (95th pct or max). Core to S1-A and S2-A | none (only a 60-min travel cap + c(t_max) stranding price) | Add a **max-catchment cap** constraint |
...
does not create very small city catchments. *Prerequisite fix:* `baseline_metrics` currently drops
clients with no reachable existing pharmacy (ITG ≈ 14 % of pop) while the sweep prices stranded
clients at c(t_max) — make the two consistent before computing these metrics or any
distance-to-Pareto measure.
...
| **Travel cost functions** | linear · quadratic/exponential · flat-then-linear · logistic (preferred, kinks ~5 & ~45 min). Quadratic risks a pharmacy next to one island household | LINEAR + LOGISTIC (midpoint 30 / scale 15); quadratic & piecewise defined but unused | **Retune logistic to ~5/~45**; add **flat-then-linear**; test/sweep sensitivity |
